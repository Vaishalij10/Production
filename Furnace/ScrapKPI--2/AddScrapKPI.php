...
/***
 * 
 * PER TON POWER CONSUMPTION CALCULATION
 */
$pertonpower_jp=number_format((float)($jp_power/$jp_prod),2,'.','');
$pertonpower_isc=number_format((float)($isc_power/$isc_prod),2,'.','');
...
$sql_scrapkpi= "INSERT INTO `scrapkpi` (`kpi_date`, 
        ...
        `sp_running_hr`, `stainlesssteel`, `aluminium`, `copper`, `bronze`,`mon_jp_prod`,
        `avg_jp_prod`,`mon_isc_prod`,`avg_isc_prod`,`per_ton_power_jp`,`per_ton_power_isc`) values
        ...
        ."'".$sp_prod."','".$sp_power."','".$sp_run_hr."','".$stainlesssteel."','".$aluminium."','".$copper."',
            '".$bronze."','".$mon_jp_prod."',"
        . "'".$avg_jp_prod."','".$mon_isc_prod."','".$avg_isc_prod."','".$pertonpower_jp."','".$pertonpower_isc."')";
...
*Power*- `$jp_power`
*Time*-  `$jp_run_hr`  
*Per Ton Powee Consumption*`$pertonpower_jp`    
...
*Power*- `$isc_power`  
*Time*-  `$isc_run_hr` 
*Per Ton Power Consumption*`$pertonpower_isc`     
...
*Total Scrap Unload*-_*`$ts_unload`*_    
  ...
  mysqli_close($conn);
header("Location: http://dataapp/furnace/Home.php");**/
exit();
