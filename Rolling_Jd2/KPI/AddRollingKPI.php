...
$Tdntimemillchill= RollingBD::getInstance()->get_prod_down_time_reason($_POST['kpidate'],62);
// total down time in hr in due mIlll chill
$Tdntimemillchillhr=$Tdntimemillchill/62;
// total bypass production due to mIlll CHILLI
$bypsprodmillchill=RollingBD::getInstance()->get_bypass_prod_reason($_POST['kpidate'],62);
$Tbypsprodmillchill=($bypsprodmillchill/$Tccmproduction)*100;
// total bypass due to mIlll CHILLI
$Tbbypassmillchill=RollingBD::getInstance()->get_billets_bypass_reason($_POST['kpidate'],62);
...
$Tperbbypass3st=RollingBD::getInstance()->billets_bypass_prod_3rdstand($_POST['kpidate']);
 ...
 * 
 * 
 * THIS IS FOR THE MESSAGE FORMATTING OF THE 24 HOURS PRODUCTION REPORT
 */
...
$NightMisrroll=RollingBD::getInstance()->get_total_missroll_of_shift($_POST['kpidate'],'Night');
...
if($prev_mon_value !=$pres_mon_value){
   ...
   $row_2 = mysqli_fetch_row(mysqli_query($link, "select monthlynettmt ,grosstmt, totalccmprod from `rollingkpi` where kpi_id in (select max(kpi_id) from rollingkpi)"));
   $prev_grosstmt=$row_2[1];
   $prev_ccmprod=$row_2[2];
   $prev_nettmt=$row_2[0];
   echo $NETTMT; echo "<br>";
   echo $prev_nettmt; echo "<br>";
   echo $prev_grosstmt; echo "<br>";
   echo $prev_ccmprod; echo "<br>";
  $monthlynettmt= number_format((float)($prev_nettmt+$NETTMT),2,'.','');
  echo "monthlynettmt"; echo "<br>"; echo $monthlynettmt; echo "<br>";
   $monthlyhotrolling= number_format((float)(($prev_grosstmt+$TROLLINGPROD)/($prev_ccmprod+$Tccmproduction)*100),2,'.','');
echo $monthlyhotrolling;
...
$sql_kpi= "INSERT INTO `rollingkpi` ( `kpidate`, `heatcount`, `grosstmt`, `totalccmprod`, `heatruntime`, `pertonpowerunits`, 
        ...
        `pertonpowerconsumption`,`endcutmrwt`,`Tperbypass3st`,`Tpermillmissrollccm`) 
          VALUES 
        ...
        . "'".$Tbypassduetomill."','".$Tbypassduetoccm."','".$Tdntimemopenmin."','".$Tdntimemopenhr."','".$Tbbypassmopen."',"
        . "'".$Tbypsprodmopen."','".$pertonpower."','".$endcutmrwt."','".$Tperbypass3st."','".$Tpermrprodmillccm."')";
